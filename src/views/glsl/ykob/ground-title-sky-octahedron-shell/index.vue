...
import skyOctahedronVertexShellShader from './skyOctahedronShell-vs.glsl'
import skyOctahedronFragmentShellShader from './skyOctahedronShell-fs.glsl'
...
export default {
  ...
  mounted () {
    ...
    let ground, title, skyOctahedron, skyOctahedronShell
    let now
    function init () {
      ...
      {
        const skyOctahedronGeometryShell = new THREE.OctahedronBufferGeometry(150, 4)
        skyOctahedronShell = new THREE.Mesh(skyOctahedronGeometryShell,
          new THREE.RawShaderMaterial({
            uniforms: {
              time: {
                type: 'f',
            ...
            vertexShader: skyOctahedronVertexShellShader,
            fragmentShader: skyOctahedronFragmentShellShader,
            transparent: true,
            side: THREE.DoubleSide,
            depthWrite: false
          }))

          scene.add(skyOctahedronShell)
      }
    ...
    function animate (time) {
      ...
      skyOctahedronShell.material.uniforms['time'].value = now
      render()
    }
    function render () {
