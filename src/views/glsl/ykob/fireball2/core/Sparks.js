...
import { radians, spherical } from '@/utils/math-ex'

import vs from './shaders/sparks.vs.glsl'
import fs from './shaders/sparks.fs.glsl'
...
export default class Sparks extends THREE.InstancedMesh {
  ...
  constructor() {
    ...
    for (let i = 0, ul = COUNT; i < ul; i++) {
      const radian1 = radians((Math.random() * 2 - 1) * 75)
      const radian2 = radians(Math.random() * 360)
      const radius = 1
      const sphericals = spherical(radian1, radian2, radius)

      ibaPositions.setXYZ(i, sphericals[0] * 5, sphericals[1] * 5, sphericals[2] * 5)
      ibaDirections.setXYZ(i, sphericals[0], sphericals[1], sphericals[2])
