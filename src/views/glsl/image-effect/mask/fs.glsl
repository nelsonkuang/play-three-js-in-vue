...
const vec2 touchPoint = vec2(500.0, 600.0);
...
void main() {
  ...
  vec2 pos = v_texCoord;
  pos.y *= aspe; // 因为要乘以一个比例，所以上面要把坐标范围从 [0, 1] 变为 [-1, 1]?  我认为是没有必要的
  vec2 centre = (touchPoint.xy / intXY.xy) * 2.0 - 1.0;
  centre.y *= aspe;
  vec4 tc = texture2D(s_baseMap, vec2(v_texCoord.x, 1.0 - v_texCoord.y)); // 这里纹理坐标应该是有问题的，应该用原始 [0, 1]的值(gl_FragCoord.xy / resolution.xy)
